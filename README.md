...
| 操作 | 行为 |
|------|------|
| **按下 Fn 键** | 提示音后开始录音（来自麦克风） |
| **松开 Fn 键** | 录音结束 → 本地识别 → 上屏 |
| 录音中再次按 Fn | 忽略（防止重复触发） |

> **注意**：部分蓝牙键盘或轻薄本的 Fn 键是纯硬件修饰键，不产生独立按键事件。如果你按 Fn 键无反应，请参考下方「诊断」章节。

### 运行流程

1. **按键检测** — 引擎收到 Fn 按键事件（keycode 190 / KEY_F20）
...
4. **ONNX 推理** — `transcribe()` 将特征送入 Paraformer 模型（`session.Run`），输出 logits → argmax 解码 → token 合并 → 标点预测
...
如果按 Fn 键语音无反应，请按以下步骤排查：

```bash
# 1. 查看调试日志
tail -f /tmp/vocotype-voice.log

# 2. 用 wev 检测 Fn 键是否产生事件（Wayland）
wev | grep -A3 "key:"
...
VoiceInput: keyval=0(0x0) keycode=190 modifiers=0x9 is_fn=1
...
VoiceInput: result='今天天气怎么样'
```

如果 Fn 键不产生事件，可修改 `src/PYVoiceInput.cc` 中的 `fn_keycode` 常量，替换为其他可用键的 keycode（建议：右 Ctrl / Scroll Lock / Menu 键）。
